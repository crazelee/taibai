...
func (m *WxConfig) GetWxAccessToken(code string) (accessToken *WxAccessToken, err error) {
	...
	body, err := utils.NewRequest("GET", wechat.AccessTokenURL, []byte(params.Encode()))
	if err != nil {
		return accessToken, err
	}
...
func (m *WxAccessToken) GetUserInfo() (wxUserInfo *WxUserInfo, err error) {
	...
	body, err := utils.NewRequest("GET", wechat.UserInfoURL, []byte(params.Encode()))
	if err != nil {
		return nil, err
	}
...
func (m *WxAccessToken) GetRefreshToken(appid string) error {
	...
	if appid == "" {
		return errors.New(consts.ErrAppIDEmpty)
	}

	if m.RefreshToken == "" {
		return errors.New(consts.ErrRefreshTokenEmpty)
	}

	if m.OpenID == "" {
		return errors.New(consts.ErrOpenIDEmpty)
	}
	...
	body, err := utils.NewRequest("GET", wechat.RefreshTokenURL, []byte(params.Encode()))
	if err != nil {
		return err
	}
...
func (m *WxAccessToken) CheckAccessToken() (ok bool, err error) {
	...
	if m.AccessToken == "" {
		return ok, errors.New(consts.ErrAccessTokenEmpty)
	}

	if m.OpenID == "" {
		return ok, errors.New(consts.ErrOpenIDEmpty)
	}
	...
	body, err := utils.NewRequest("GET", wechat.CheckAccessTokenURL, []byte(params.Encode()))
	if err != nil {
		return ok, err
	}
...
func (m *WxConfig) GetJsCode2Session(code string) (wXBizDataCrypt *WXBizDataCrypt, err error) {
	...
	body, err := utils.NewRequest("GET", wechat.JsCode2SessionURL, []byte(params.Encode()))
	if err != nil {
		return wXBizDataCrypt, err
	}
